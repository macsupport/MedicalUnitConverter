...
<NavLeft><Link href="/about/" ><i class="fa-fw fad fa-info-circle fa-2x fa-fw color-white"></i>
                                    </Link></NavLeft>

 <div center class="text-2xl"><i class="fad fa-laptop-medical fa-fw fa-2x pr-2 fadrop" style="--fa-secondary-opacity: 1.0; --fa-primary-color: white; --fa-secondary-color: silver;"></i> <span class="color-white">Medical Unit Converter</span></div>
 
 
 <NavRight>
      
      <Button class="helpme" popupOpen=".help-popup"> <i class="fa-fw fad fa-question-circle fa-2x fa-fw color-white"></i>
                                    </Button>                             
                                    </NavRight>
...
<Toolbar class="hidden-print" tabbar icons position="top">
    ...
    <Link tabLink="#vet1" text="Veterinary Medical">
                                                <i class="mb-1 fa-2x fad fa-paw" style=" --fa-primary-color: black;--fa-secondary-color: white;"></i> 
                                                 </Link>
    <Link
      tabLink="#metric1"
...
<Tabs>
    <Tab id="human1" class="page-content" tabActive>
       ...
       <form data-search-container=".virtual-list" data-search-item="li" data-search-in=".name"
...
<div class="w-full mx-auto mx-1 bg-color-white print:w-full"><div class="flex md:w-5/6 mx-auto py-1 print:w-full"><div class="w-1/2 md:w-2/3 text-black font-semibold text-lg">  Human Lab Values <div class="subtitle">SI <i class="fad fa-arrows-alt-h fa-lg hidden-print"></i> US Units</div></div>
...
<div class="text-sm"></div></Link>
  <a class="resetme hidden-print" href="#"  on:click={() =>{reset()}}><span class="refresh-link refresh-home"> <i class="fad fa-sync-alt fa-2x fa-fw mt-2"></i></span></a>    <a class="printme hidden-print" data-print="human" href="#"><i class="fad fa-print fa-2x fa-fw mt-2"></i></a></div>
...
<hr/>
 ...
 <div class="flex w-full">
  <List
    class="searchbar-found virtual-list mx-auto w-full md:w-3/4"
    ul={false}
    virtualList
    virtualListParams={{
      items: items,
  ...
  >
    <ul class="virtual list-none divide-y divide-solid divide-gray-400">
      {#each vlData.items as item, index (index)}

       <li class="unit py-1"
          link="#"
          title={item.name}
          subtitle={item.specimen}
          style={`top: ${vlData.topPosition}px`}
          virtualListIndex={items.indexOf(item)}
          >
<div class="flex flex-wrap py-1">
 <div class="xs:w-full sm:w-1/2 print:w-1/2" >
            <div class="item-name font-bold text-black text-md">{@html item.name}</div>
...
{#if item.alternate === "true"}<input class="input-us md:w-2/3 drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none" name="us{filteredList.indexOf(item)}" data-original-value={item.originalusValue} value={item.usValue} on:input="{e => setBothFromUSH2(e.target.value, filteredList.indexOf(item))}"  type=number placeholder=" US"/>{:else}<input class="input-us md:w-2/3 md:inline drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none" name="us{filteredList.indexOf(item)}" data-original-value={item.originalusValue} value={item.usValue} on:input="{e => setBothFromUSH(e.target.value, filteredList.indexOf(item))}"  type=number placeholder=" US"/>
{/if}</div>
<div class="xs:w-1/2 sm:w-1/4 sm:px-1 print:w-1/4">
 <label class="text-gray-900 text-xs mb-1" for="grid-first-name">
        SI Range: {@html item.siRange} {@html item.siUnit}
      </label>
      {#if item.alternate === "true"} <input class="input-si md:w-2/3 drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none" name="si{filteredList.indexOf(item)}" data-original-value={item.originalsiValue} value={item.siValue} on:input="{e => setBothFromSIH2(e.target.value, filteredList.indexOf(item))}" type=number placeholder="SI"/>{:else}<input class="input-si md:w-2/3 md:inline drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none" name="si{filteredList.indexOf(item)}" data-original-value={item.originalsiValue} value={item.siValue} on:input="{e => setBothFromSIH(e.target.value, filteredList.indexOf(item))}" type=number placeholder="SI"/>{/if}
</div>
</div> 
...
<!-- <div id="human" class="divide-y divide-gray-300 w-full printthis px-1 holderx md:w-5/6 md:mx-auto print:w-full">
...
<form class="searchbar searchbar-enabled" data-f7-slot="fixed"> <div class="searchbar-inner"> <div class="searchbar-input-wrap"> <input placeholder="Search" type="search" class="" bind:value={searchTermV}/> <i class="searchbar-icon"></i> <span class="input-clear-button"></span> </div>   </div> </form> </div>
             <div class="w-full mx-auto mx-1 bg-color-white print:w-full"><div class="flex md:w-5/6 mx-auto py-1 print:w-full"><div class="w-1/2 md:w-2/3 text-black font-semibold text-lg">  Veterinary Lab Values<div class="subtitle">SI <i class="fad fa-arrows-alt-h fa-lg hidden-print"></i> US Units</div></div>
...
<hr/>
<div id="vet"  class="divide-y divide-gray-300 printthis px-1 holderx md:w-5/6 md:mx-auto print:w-full">
<div class="xs:hidden sm:hidden md:inline w-full px-2 mb-2 shadow md:flex md:flex-no-wrap md:mx-auto print:block bg-gray-300 dark:bg-gray-700">
...
{/each}
  ...
  <form class="searchbar searchbar-enabled" data-f7-slot="fixed"> <div class="searchbar-inner"> <div class="searchbar-input-wrap"> <input placeholder="Search" type="search" class="" bind:value={searchTermM}> <i class="searchbar-icon"></i> <span class="input-clear-button"></span> </div>   </div> </form></div>

    <div class="w-full mx-auto mx-1 bg-color-white print:w-full"><div class="flex md:w-3/4 mx-auto py-1 print:w-full"><span class="w-1/2 md:w-2/3 text-black font-semibold text-lg">  Metric Unit Conversion</span>
<div class="w-1/2 md:w-1/3 flex" style="justify-content: space-around;"><Link class="ml-2 hidden-print" onClick={createSheet}> <i class="fad fa-calculator fa-2x fa-fw"></i>
                                    <div class="text-sm"></div></Link>
  <a class="resetme hidden-print" href="#"  on:click={() =>{reset()}}><span class="refresh-link refresh-home"> <i class="fad fa-sync-alt fa-2x fa-fw"></i></span></a>    <a class="printme hidden-print" data-print="metric" href="#"><i class="fad fa-print fa-2x fa-fw"></i></a></div></div> 
...
<div id="metric" class="divide-y divide-gray-300 printthis unitsMetric px-1 md:w-3/4 mx-auto print:w-full"> 
<div class="xs:hidden sm:hidden md:inline w-full md:w-3/4 px-2 mb-2 shadow md:flex md:flex-no-wrap mx-auto print:block bg-gray-300 dark:bg-gray-700">
 ...
 <!-- <VirtualList bind:start bind:end items={filteredListM} let:item > -->
    {#each filteredListM as item, i}
 <div class="units py-1 mb-1 print:mb-0 print:py-0">
<div class="flex flex-wrap py-1">
 <div class="flex w-full md:w-2/3" >
            <div class="w-2/3 font-bold text-black text-base name md:mt-2">{item.conventionalUnit} <i class="fad fa-arrows-alt-h fa-lg color-green"></i> {item.siUnit} <span style="display:none;">{item.name}</span></div>
...
<div class="flex w-full md:w-1/3" >
  ...
  <label class="text-gray-700 text-xs font-bold" for="grid-first-name">
 ...
 <div class="w-1/2 sm:px-1">
 <label class="text-gray-700 text-xs font-bold" for="grid-first-name">SI - {item.siUnit}</label>
...
{/each}
<!-- </VirtualList> -->
<!-- <p class="text-base">showing items {start +1}-{end+1} of {filteredListM.length}</p> -->
 </div> 
...
// A1C(%) = (A1C(mmol/mol) / 10.929) + 2.15
 // convtommol = (10.929 * (var_hba1c - 2.15)).toFixed(1);
 //        convtohba1c = (2.15 + (var_mmol / 10.929)).toFixed(1);
