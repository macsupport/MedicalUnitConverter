...
{#if item.alternate === "true"}<input class="input-us md:w-2/3 drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none"   data-original-value={item.originalusValue} bind:value={item.usValue} on:input={e => setBothFromUSH2(e.target.value, i)}  type="number" placeholder=" US"/>{:else}<input class="input-us md:w-2/3 md:inline drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none"  data-original-value={item.originalusValue} bind:value={item.usValue} on:input={e => setBothFromUSH(e.target.value, i)}  type="number" placeholder=" US"/>
{/if}</div>
<div class="xs:w-1/2 sm:w-1/4 sm:px-1 print:w-1/4">
 <label class="text-gray-900 text-xs mb-1" for="grid-first-name">
        SI Range: {@html item.siRange} {@html item.siUnit}
      </label>
      {#if item.alternate === "true"} <input class="input-si md:w-2/3 drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none"  data-original-value={item.originalsiValue} bind:value={item.siValue} on:input={e => setBothFromSIH2(e.target.value, i)} type="number" placeholder="SI"/>{:else}<input class="input-si md:w-2/3 md:inline drug appearance-none mt-2 block w-full bg-gray-100 text-gray-900 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:border-gray-500 print:inline print:px-1 print:py-1 print:mt-0 print:w-1/2 print:border-none"   data-original-value={item.originalsiValue} bind:value={item.siValue} on:input={e => setBothFromSIH(e.target.value, i)} type="number" placeholder="SI"/>{/if}
</div>
</div>  
</div>
